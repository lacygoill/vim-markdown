...
def markdown#fold#how_many#print() #{{{2
    if foldclosed('.') == -1
        var first_line = search('^#', 'bcnW')
        var last_line = search('^#\|\%$', 'cnW')
    ...
    MoveToFirstFold(1)
    var msg_first_part = GetNumberOfFolds(1)
    winrestview(view)

    # get the number of folds with the same level,
    # and the same parent fold
    MoveToFirstFold(2)
    var msg_second_part = GetNumberOfFolds(2)
    winrestview(view)
...
def GetNumberOfFolds(n: number): number #{{{2
    var pat = current_lvl == 1
        ? '^#\+$\|\%$'
        : (n == 1 ? '^#\+$\|' : '') .. '^#\{' .. (current_lvl - 1) .. '}#\@!\|\%$'

    var first_line_last_fold = search(pat, 'nW')
    var cnt = 0
...
def GetCurrentLvl(): number #{{{2
    var pat = '^#\+\ze\s\+'
    var first_line = search(pat, 'bcnW')
